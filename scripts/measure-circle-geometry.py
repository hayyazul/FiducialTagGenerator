...
def measure(name: str, tile_px: int, valid: int) -> tuple[float, list[tuple[int, bool]]]:
    ...
    return max_radius, cands


def main() -> None:
    print("Family               tile_px   validTags   outerRadius_modules   border candidates (side,colour)")
    print("-" * 110)
    results: list[tuple[str, int, int, float, list[tuple[int, bool]]]] = []
    for name, tile_px, valid in CIRCLE_FAMILIES:
        radius, cands = measure(name, tile_px, valid)
        results.append((name, tile_px, valid, radius, cands))
        cands_str = ", ".join(
            f"{side}/{'B' if black else 'W'}" for side, black in cands
        ) or "(none)"
        print(f"{name:<20} {tile_px:>7}   {valid:>9}   {radius:>19.6f}   {cands_str}")
    ...
    for name, tile_px, valid, radius, _cands in results:
        ...
        print(f"    outerRadius_modules: {radius:.6f},")
